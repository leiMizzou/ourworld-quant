...
# 2026-07 基于 2018→2026 hfq 全窗证据重选:在前 70% 调仓期(2018-01→2023-12)上比较
# 5 个候选组合,amihud+低波动 训练窗最优(Sharpe 0.76/回撤最小/换手最低),
# 后 30% 测试窗一次性验证成立(Sharpe 0.74)。旧组合 reversal+低波动 在两个窗口都亏损。
# 注意:amihud 买的是低流动性股票(非流动性溢价),纸面收益在真实交易中受容量约束,
# 教学/模拟用途可接受;2024→2026 小盘牛市中该组合跑输全池等权基准(见报告基准行)。
DEFAULT_SPECS = [
    ("amihud", 20, +1),       # 非流动性溢价:成交额相对价格波动越"薄"越买
