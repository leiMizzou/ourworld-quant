...
# 例:
#   ./setup.sh ourworld.app quant myname "Lei"
#   -> 站点域名 quant.ourworld.app, GitHub 用户 myname
set -euo pipefail

DOMAIN="${1:?请提供主域名,如 ourworld.app}"
SUB="${2:-quant}"
GH="${3:?请提供 GitHub 用户名}"
NAME="${4:-OurWorld Quant Lab}"
FQDN="${SUB}.${DOMAIN}"
...
cat <<EOF
  ...
  git init
  git add .
  git commit -m "chore: initial commit — OurWorld Quant Lab"
  git branch -M main
