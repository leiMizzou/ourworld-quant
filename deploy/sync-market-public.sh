...
REPORT_SOURCE="${OWQ_REPORT_SOURCE:-$DATA_SOURCE}"
...
SYNC_REPORT_DATA="${OWQ_SYNC_REPORT_DATA:-0}"
...
if [[ "$SYNC_DATA_FIRST" == "1" ]]; then
  ...
  "$PYTHON" -m src.data.cli daily \
    ...
    --limit "$MARKET_LIMIT"
  if [[ "$SYNC_REPORTS" == "1" && "$SYNC_REPORT_DATA" == "1" && ( "$REPORT_ADJUST" != "$DATA_ADJUST" || "$REPORT_UNIVERSE_STATUS" != "$DATA_UNIVERSE_STATUS" ) ]]; then
    "$PYTHON" -m src.data.cli daily \
